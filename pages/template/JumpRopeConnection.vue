	...
	export default {
		...
		data() {
			return {
				...
				communicationType:"\"ICDeviceCommunicationTypeConnect\""
			};
		},
		async onLoad(e) {
		...
		methods: {
			...
			cancelBind() {
				...
				bluethModule.removeDevices(params, (result) => {
			...
			connect(data) {
				...
				uni.setStorageSync('macAddr',data.deviceId)
				uni.setStorageSync('name',data.name)
				uni.setStorageSync('communicationType',_this.communicationType)
