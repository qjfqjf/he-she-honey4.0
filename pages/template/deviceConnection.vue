  ...
  <view class="content p-2">
    ...
    <view class="deviceList">
      <view class="header d-flex j-sb a-center">
        <view class="desc"> 可用设备 </view>
        <view class="" @click="resetDevice">
          <image
            src="@/static/icon/device/reset.png"
      ...
      >
        <view class="mr-3 mt-2">
          <image
            ...
            src="@/static/icon/device/ble.png"
            mode="aspectFit"
          >
          </image>
        </view>
...
<script>
  ...
  export default {
    ...
    mounted() {
      uni.$on('back', this.onPageBack)
    },
    beforeDestroy() {
      // 移除页面返回事件监听
      uni.$off('back', this.onPageBack)
    },
    onUnload() {
      this.stopDiscovery()
    },
    methods: {
      ...
      cancelBind() {
        ...
        this.deviceInfo.deviceId = uni.getStorageSync(this.deviceInfo.dName)
        console.log(uni.getStorageSync(this.deviceInfo.dName))
        uni.$emit('updateDevice', this.deviceInfo)
      },
      // 重新搜索设备
      resetDevice() {
        this.discovery()
      ...
      connect(data) {
        ...
        uni.createBLEConnection({
          ...
          success(res) {
            ...
            uni.$emit('updateDevice', _this.deviceInfo)
          },
          fail(err) {
            console.log('连接失败')
            console.error(err)
