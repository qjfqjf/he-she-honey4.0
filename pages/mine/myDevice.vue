<template>
  <view class="content p-2">
    ...
    <view class="d-flex j-sb flex-wrap">
      <view
        class="bg-purple-light m-2"
        v-for="(item, index) in baseList"
        @click="click(item.url, item.title + item.detail, item.deviceInfo)"
        :key="index"
      >
        <u-badge
          class="mb-2"
          numberType="overflow"
          :type="item.deviceInfo.deviceId ? 'success' : 'error'"
          :value="item.deviceInfo.deviceId ? '已绑定' : '未绑定'"
        ></u-badge>
